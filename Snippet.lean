...
/-- The headline behaviour of `notApplicable`: a silent vote at the head just disappears. This is *why* `optimize` is sound. -/
...
/-- The auditor is matched *only* by an `abstain` rule: the policy speaks about it and still deliberately says nothing — a state a Bool could never capture. -/
